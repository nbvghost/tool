...
func (hms _HhMmSs) ToHhMmSsString() string {
	hH := "00" + strconv.Itoa(int(hms[0]))
	mM := "00" + strconv.Itoa(int(hms[1]))
	sS := "00" + strconv.Itoa(int(hms[2]))

	return hH[len(hH)-2:] + ":" + mM[len(mM)-2:] + ":" + sS[len(sS)-2:]
}
